...
class CameraCapture(object):
    ...
    def __init__(
        ...
        if self.videoPath.startswith("rtsp"):
        ...
        if self.resizeWidth != 0 or self.resizeHeight != 0:
            self.nbOfPreprocessingSteps +=1
        if self.verbose:
            print("Initializing the camera capture with the following parameters: ")
    ...
    def __sendFrameForProcessing(self, frame):
        ...
        try:
            endpoint = self.imageProcessingEndpoint
    ...
    def start(self):
        ...
        while self.keepRunning:
            try:
                ...
                if self.isWebcam:
                    ...
                    if frameCounter == 1:
                        if self.capture.get(cv2.CAP_PROP_FRAME_WIDTH) < self.capture.get(cv2.CAP_PROP_FRAME_HEIGHT):
                            print("Setting autorotate to true.")
                            self.autoRotate = True
                    if self.autoRotate:
                        print("Autorotate is on!")
                ...
                if self.isWebcam and self.nbOfPreprocessingSteps == 1 and (self.resizeWidth != 0 or self.resizeHeight != 0):
                    preprocessedFrame = cv2.resize(frame, (self.resizeWidth, self.resizeHeight))

                if self.isWebcam and self.nbOfPreprocessingSteps > 1:
                    preprocessedFrame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                ...
                if self.showVideo:
                    try:
                        self.displayFrame = encodedFrame
                ...
                print('__start Camera capture Exception -' + str(e))
